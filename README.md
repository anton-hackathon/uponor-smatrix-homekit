...
Читает и пишет шину термостатов Uponor и управляется через **HomeKit** / приложение «Дом» для iPhone напрямую, без Home Assistant или покупки модуля Uponor R208 за 400 eur.
...
3. **Направление TXD/RXD перекрёстное** — выход модуля идёт на вход ESP32, см. таблицу. Питание модуля RS-485 от **3.3 В** платы.
...
**Питание.** Обычная USB-зарядка 5 В в разъём платы. Важно: **USB-C зарядник в розетку часто не может запитать эту плату** (согласование питания USB-C), а USB-A 5 В или 5 В на пин VIN — получается почти всегда.
...
Поправку `UPONOR_TARGET_TEMP_OFFSET` можно оставить **2**, это сдвиг, который вносит сам UPONOR (см их документацию).
