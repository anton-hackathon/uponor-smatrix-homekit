...
Расшифровка битов `0x3F` подтверждена независимо — анализом протокола от автора
реверс-инжиниринга ([kroimon/uponor-smatrix-analysis](https://github.com/kroimon/uponor-smatrix-analysis)):
бит `0x0400` — «разрешено охлаждение», бит `0x0800` — «разрешён нагрев». Разложим
наши значения:

| Зона | `0x3F` | `0x0800` нагрев | `0x0400` охлаждение |
|---|---|---|---|
| рабочие | `1C03` | ✅ | ✅ |
| санузлы | `1803` | ✅ | ❌ |

Важный вывод: у санузлов сброшен только бит охлаждения, а **бит нагрева стоит**.
Значит запрет касается лишь охлаждения, и зимой, когда система перейдёт в нагрев,
санузлы заработают как обычные зоны. Пока система в охлаждении, они правомерно
простаивают.
...
То есть знак отката зависит от режима: в нагреве уставка уменьшается, в
охлаждении увеличивается. В коде это учтено — откат применяется со знаком по
текущему режиму (функция `ecoDeltaRaw_` в `uponor_zone.cpp`).

Механика подтверждена независимо — логами протокола от автора реверс-инжиниринга
([kroimon/uponor-smatrix-analysis](https://github.com/kroimon/uponor-smatrix-analysis),
сессия в режиме нагрева): при включении ECO регистр `0x3B` на шине **не меняется** —
в нём всегда лежит базовая (comfort) уставка, а термостат применяет откат сам.
Поэтому прошивка вычисляет эффективную уставку для «Дома» как «база ± откат», а не
ждёт готового значения с шины. У автора этой прошивки ECO пока не был активен
(`0x3E = 0000` у всех зон), так что на живой системе поведение ещё предстоит
сверить, но модель и знак совпадают и с документацией, и с логами kroimon.
...
установку. По шине видно: бит `0x0400` («разрешено охлаждение») в регистре `0x3F`
сброшен (`1803` вместо `1C03`), `0x35 = 0000`. Запись установки **по шине проходит** —
блокировка только на физическом пульте. При этом бит `0x0800` («разрешён нагрев»)
у санузлов стоит (расшифровка битов подтверждена анализом kroimon), поэтому зона
снова станет активной, когда система переключится в HEAT.
