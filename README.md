...
Читает и пишет шину термостатов Uponor и отдаёт управление в **HomeKit**. Приложение «Дом» на iPhone
управляет уставками напрямую, без Home Assistant или покупки модуля Uponor R208.

Исходный проект был собран на базе ESPHome, откуда взяты знание о протоколе uponor, плюс использованы наработки HOMESpan.
...
Если ESP32 питается от ноутбука, а контроллер от сети — нужен ещё провод от GND
модуля на клемму земля X-245: без нее приёмнику не сравнить A и B.
...
1. **Полярность A/B.** Самая частая ошибка. Даёт ровно ноль принятых байтов
   (`rx=0`). Перепутать местами **безопасно** — RS485 от этого не
...
3. **Направление TXD/RXD перекрёстное** — выход модуля идёт на вход ESP32, см.
   таблицу. Питание модуля от **3.3 В** платы.

Признак живого железа: светодиод RXD на модуле **мигает сам по себе** ещё до
прошивки — X-245 непрерывно опрашивает термостаты. Не мигает → проблема, скорее всего, в перепутанных A/B, загружать прошивку рано.

**Питание.** Обычная USB-зарядка 5 В в разъём платы. Важно: **USB-C зарядник в
розетку часто не может запитать эту плату** (согласование питания USB-C), а USB-A 5 В или
5 В на пин VIN — получается почти всегда.
...
Поправку уставки `UPONOR_TARGET_TEMP_OFFSET` оставь **2**, это сдвиг, который вносит сам UPONOR (см их документацию)
