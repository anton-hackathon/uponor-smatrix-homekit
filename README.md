...
Позволяет управлять через **HomeKit** (приложение «Дом» для iPhone) термостатом Uponor X-245, без отдельного сервера Home Assistant или покупки фирменного модуля Uponor R208 за 400 eur. Исходно собран на базе /esphome/esphome ESPHome, откуда взяты знания о протоколе uponor, и потом добавлены наработки HOMESpan. Делалось через Claude Code, не спрашивайте меня детальнее, как оно устроено - я не в курсе 😆
