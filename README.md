# ESP32 + RS485 → Uponor Smatrix Base Pulse X-245

Читает и пишет шину термостатов Uponor и отдаёт пять комнат в **HomeKit** через
[HomeSpan](https://github.com/HomeSpan/HomeSpan) — приложение «Дом» на iPhone
управляет уставками напрямую, без Home Assistant, домена и SSL.

Раньше проект был собран на ESPHome; знание протокола, добытое
реверс-инжинирингом, ниже актуально и для текущей версии.
...
| Опрос | X-245 сам поочерёдно опрашивает термостаты, ESP32 слушает и вклинивается только при записи уставки |

Подключаться можно к клеммам **A/B** как на самом X-245, так и на любом термостате — шина общая.

## Железо (определено по факту)
...
| Модуль | Куда | Примечание |
|---|---|---|
| TXD | ESP32 **GPIO16** | выход модуля → вход ESP32 (перекрёстно!) |
| RXD | ESP32 **GPIO17** | вход модуля ← выход ESP32 |
| VCC | ESP32 **3.3V** | тогда TXD качает 3.3 В — вход ESP32 в безопасности |
| GND | ESP32 **GND** | |
| A | клемма **A** на X-245 | |
| B | клемма **B** на X-245 | |

Если ESP32 питается от ноутбука, а контроллер от сети — проведите ещё провод от GND
модуля на земляную клемму X-245: без общей опоры приёмнику нечем сравнивать A и B.
...
Пошаговый порядок для новой системы. Диагностика каждого шага — в разделах
...
- **ESP32** — проверено на классической плате с чипом WROOM-32 и 4 МБ флеша
  (в PlatformIO это `board = esp32dev`). Другие ESP32 с HomeSpan тоже работают,
  но это самый дешёвый и обкатанный путь.
- **RS485-модуль с аппаратным автопереключением направления** (без пинов DE/RE) —
  например DollaTek. Заметно упрощает: не нужен flow-control пин.
...
1. **Полярность A/B.** Самая частая ошибка. Даёт ровно ноль принятых байтов
   (`rx=0`), а не мусор. Перепутать местами **безопасно** — RS485 от этого не
   горит; если байтов нет, первым делом поменяй A и B.
2. **Общая земля.** Если ESP32 и X-245 питаются от разных источников, проведите
   провод от GND модуля на земляную клемму X-245. Без общей опоры приёмнику не с чем
   сравнивать A и B — тоже `rx=0` или сплошные `00`/`FF`.
3. **Направление TXD/RXD перекрёстное** — выход модуля идёт на вход ESP32, см.
   таблицу. И питание модуля от **3.3 В** платы, чтобы его выход не подавал 5 В на
   вход ESP32.

Признак живого железа: светодиод RXD на модуле **мигает сам по себе** ещё до
прошивки — X-245 непрерывно опрашивает термостаты. Не мигает → проблема чисто в
железе, загружать прошивку рано.

**Питание.** Обычная USB-зарядка 5 В в разъём платы. Важно: **USB-C зарядник в
розетку часто не заводит эту плату** (согласование питания USB-C), а USB-A 5 В или
5 В на пин VIN — заводят всегда.
...
- **[src/config.h](src/config.h)** — всё остальное: пины RS485, часовой пояс,
  имя устройства, поправка уставки и таблица зон. Адреса термостатов пока
  оставь как есть — их найдём на следующем шаге.

Поправку уставки `UPONOR_TARGET_TEMP_OFFSET` поставь **0** — она подбирается позже
(шаг 6). Значение `2.0` в репозитории — калибровка авторской системы, у тебя
почти наверняка другое.
...
из `src/secrets.h`. Появится мост и по одному термостату на комнату — с уставкой,
...
Открой панель термостата в «Доме» и сравни уставку с экраном физического пульта в
той же комнате. Если расходятся на постоянную величину — это и есть
`UPONOR_TARGET_TEMP_OFFSET` (у авторской системы вышло ровно +2.0 °C, методика — в
разделе «[Сдвиг уставки](#сдвиг-уставки-2-c)»). Впиши измеренное в config.h и
...
Раз в 15 с прошивка печатает состояние всех пяти зон:
...
Изменение уставки в «Доме» печатает отправленный пакет:
...
|---|---|---|
| `0xAABBCC01` | Living | **подтверждено** — ручная смена уставки отразилась на шине |
| `0xAABBCC02` | Bedroom | **подтверждено** — запись 21.5 совпала с экраном термостата |
| `0xAABBCC03` | Study | выведено: единственная уставка, не двигавшаяся ни разу |
...
целевое значение на экране не посмотреть. Их уставки, 30.5 и 29.5 на экране, оставлены
...
**Запись проверена.** Смена уставки на 16.0 °C отправила в шину

```
>>> AA BB CC 01 3B 00 00 3B 02 60 2F A5
```

`3B` — регистр уставки, `0x0260` = 608 = 60.8 °F = 16.0 °C, затем контрольная сумма.
...
Связь с точкой доступа может быть нестабильной: при первом подключении ESP32 дважды получил
`Authentication Failed` и поднялся только с третьей попытки, задержка отклика колеблется от 7 до 113 мс,
а первая загрузка по OTA сорвалась на `No route to host` и прошла со второй попытки. Пароль
при этом верный. Если OTA срывается — повтори команду.
...
Проверено экспериментом: Bathroom1 выставили уставку 22.0 при комнатных 25.3 —
превышение 3.3 °C, а зона всё равно осталась в простое. По мануалу охлаждение
запускается, когда в комнате примерно на **1 °C выше уставки** (при отключённом
autobalancing), так что порог был пройден с запасом — значит дело именно в запрете,
а не в гистерезисе. Запись уставки по шине при этом **проходит и удерживается** —
...
Сдвинут только регистр уставки. Проверено, что **не** сдвинуты:
...
Поэтому поправка применяется только на пути уставки (чтение и запись), а общая
...
### Самопроизвольная смена уставок

Один раз уставки трёх активных зон изменились сами: Living 23.0 → 19.5, Bedroom
21.5 → 19.0, Study 25.0 → 18.5. У двух простаивающих (Bathroom2, Bathroom1) не изменилось
ничего. Плата в этот момент не передавала в шину ни байта: синхронизация времени не
сработала ни разу — источником времени был Home Assistant, время было невалидно, и на
это есть проверка. В версии на HomeSpan время берётся из NTP (`configTzTime` в
`src/main.cpp`), так что этот путь стал рабочим.

После этого значения стабильны. Причина такой самопроизвольной смены — расписание
comfort/eco в самих термостатах (см. FAQ) и/или heating/cooling offset при
переключении режима (см. ниже). Для интеграции это безвредно: она отслеживает то,
что сообщает контроллер.
...
Всё сходится: в регистре `0x3B` лежит **базовая** уставка, а термостат в режиме
...
говорит — при переключении обратно в нагрев значение **уменьшает** уставку. А в
...
устройства. Верхний край проверен, 35 проходит. Официальный диапазон уставки —
...
### Настройки термостата по мануалу (что влияет на нашу интеграцию)

Меню термостата (T-148/T-149; вход — удержание OK ~3 с). Пункты, которые
объясняют поведение, которое мы наблюдали на шине:

| Меню | Что это | Значение для нас |
|---|---|---|
| `00 Program` | расписание Comfort/ECO, 7 программ (P1–P6 + пользовательская), интервалы 30 мин, **по умолчанию Off** | источник «самопроизвольных» смен уставки; хранится в термостате, по шине не транслируется |
| `02 Heating/cooling changeover` | **скрыто**, если термостат на контроллере | подтверждает: режим задаёт система, не комната |
| `03 ECO mode setback` | откат ECO, по умолчанию **4 °C**, диапазон 0–11 | регистр `0x3C`; в нагреве вычитается, в охлаждении **прибавляется** |
| `04 Control mode` | RT / RFT / RS / RO — какие датчики задействованы | у нас RT (только комнатный): `0x41` = `0x7FFF` |
| `05` / `06` | границы температуры пола | видны только при RFT — у нас нет |
| `07 Cooling allowed` | **разрешено ли охлаждение в комнате** | причина «нерабочих» санузлов |
| `08 Display unit` | °C / °F на экране | на шину не влияет: там всегда десятые °F |
| `10 Time and date` | время; нужно для расписаний | см. ниже про мастера времени |
| `11 Room temperature calibration` | калибровка **отображаемой** температуры, ±6 °C, по умолчанию 0 | **это НЕ наш offset** — правит показ температуры, а не уставку |

Пункт `11` стоит отметить особо: README раньше предполагал, что сдвиг +2 может быть
«калибровкой датчика». Мануал это опровергает — калибровка меняет отображаемую
температуру комнаты, а не уставку. Наш сдвиг — это `Heating/cooling offset`.

**Мастер времени.** Мануал подтверждает логику, которую компонент реализует
эмпирически:

> If another programmable digital thermostat is registered to the room controller
> (**with a lower channel number**), or if the Uponor Smatrix PULSE app is used,
> they will push the set time and date to the thermostat.

То есть время по системе раздаёт программируемый термостат с наименьшим номером
канала — именно его адрес прошивка и вычисляет автоматически, чтобы отправлять NTP-время.

### Прочее из мануала, полезное для диагностики

- **Порог запуска.** При отключённом autobalancing охлаждение (и нагрев) стартует,
  когда комната примерно на **1 °C** отошла от уставки. Полезно, когда зона в
...
| `0x37` / `0x38` | границы уставки (5.0 / 35.0 °C) |
| `0x3B` | уставка |
...
Заготовка под публикацию. Вопросы, которые всплыли на живой системе.

### Уставка в «Доме» сама изменилась, я её не трогал — это баг?
...
лишь читает с шины *текущую* действующую уставку и отражает её. Когда расписание
в свой момент переключает период — уставка меняется, и «Дом» честно это
показывает.

### Если я задал уставку из «Дома», а есть расписание — что победит?

Расписание управляет не уставкой, а переключением **Comfort/ECO** — то есть тем,
вычитается ли из уставки ECO-откат (по умолчанию 4 °C). Уставку ты задаёшь свою, и она
остаётся; а расписание в свои интервалы применяет к ней откат.

Про приоритет мануал говорит однозначно — но именно про режим Comfort/ECO:

> As long as scheduling (program 00) is activated (not set to Off) in a
> programmable digital thermostat, no other unit is allowed to change (override)
> the Comfort/ECO mode in that room.

То есть если в комнате включена программа, никто извне (включая приложение Uponor и
нашу интеграцию) не переключит ей Comfort/ECO. Записи уставки это не запрещает —
проверено на живой системе: значение принимается и удерживается.
...
### Почему термостат в санузле не даёт сменить уставку с пульта?
...
уставку. По шине видно: бит `0x0400` в регистре `0x3F` сброшен (`1803` вместо
`1C03`), `0x35 = 0000`. Запись уставки **по шине проходит** — блокировка только на
...
### Появится ли влажность отдельной плиткой?
...
домашнего центра** (Apple TV или HomePod). Без него, по документации Apple,
пригласить получится лишь когда вы дома и в той же сети Wi-Fi.

Удалять аксессуар из «Дома», чтобы сопрячь второй телефон, **не нужно** — и после
удаления придётся заново вводить код, потому что при корректном удалении «Дом»
стирает сопряжение и на плате (это видно как `sf=1`).
...
- **управление аксессуаром** (прочитать температуру, задать уставку) — это прямое
...
### Почему уставка в «Доме» на 2 °C отличается от экрана термостата?

Это документированный параметр Uponor **«Heating/cooling offset»** (по умолчанию 2 °C):
при переключении в охлаждение система увеличивает уставки, при возврате в нагрев —
уменьшает. В регистре `0x3B` лежит базовая уставка, а термостат показывает
эффективную. Прошивка компенсирует это константой `UPONOR_TARGET_TEMP_OFFSET` в
[src/config.h](src/config.h). Подробнее — в разделе про сдвиг уставки.
...
пины и поправка уставки — **её на чужой системе сначала поставь 0** и сверь с
экраном пульта (у автора вышло 2 °C, ровно документированное значение offset по умолчанию).
